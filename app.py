...
def extract_directions_text(route_data, df_stops=None):
    """
    Extrait le texte détaillé du trajet (instructions + client/livraison) à partir de la réponse Directions,
    et affiche à chaque arrêt l'adresse et le nom du client s'ils sont disponibles.
    df_stops : DataFrame ordonné des arrêts (colonne 'ADRESSES' et 'EXPEDITEUR')
    """
    steps_html = []
    legs = route_data.get("legs", [])
    # Par défaut : pas de clients
    client_names = [None] * len(legs)
    addresses = [None] * len(legs)
    if df_stops is not None and len(df_stops) == len(legs):
        client_names = df_stops['EXPEDITEUR'].tolist()
        addresses = df_stops['ADRESSES'].tolist()

    for i, leg in enumerate(legs):
        # Affiche adresse + client en tête de chaque leg (arrêt)
        address = addresses[i] if addresses[i] else "Adresse inconnue"
        client = client_names[i] if client_names[i] else "Client inconnu"
        steps_html.append(
            f"<b>Arrêt {i+1} :</b> {address} <span class='text-blue-700'>({client})</span>"
        )
        for step in leg.get("steps", []):
            ...
            steps_html.append(
                f"<li>{instr} <span class='text-gray-500'>({distance}, {duration})</span></li>"
            )
